// The Swift Programming Language
// https://docs.swift.org/swift-book

import SwiftUI

@MainActor
enum GraphicKeyState {
  case drawing
  case waitingResult
  case success
  case error
}

@MainActor
public struct GraphicKeyView: View {

  @State private var gestureLocation: CGPoint?

  @State private var points: [CGRect] = Array(repeating: .zero, count: 9)
  @State private var selectedPoints: [Int] = []

  @State private var scalePoints: [Int] = []

  @State private var success: Bool?

  private let checkKey: (String) -> Bool

  private func color(for point: Int, isLine: Bool = false) -> Color {
    if let success {
      return success ? Color.green : Color.red
    }

    if self.selectedPoints.contains(point) || isLine {
      return Color.blue
    }

    return Color.gray
  }

  public var body: some View {
    ZStack {
      circles

      Path { path in
        guard self.selectedPoints.first != nil else { return }
        path.move(to: self.points[self.selectedPoints[0]].center)

        self.selectedPoints.forEach { selectedPoint in
          path.addLine(to: self.points[selectedPoint].center)
        }

        if let gestureLocation {
          path.addLine(to: gestureLocation)
        }

      }
      .stroke(
        self.color(for: 0, isLine: true),
        style: StrokeStyle(
          lineWidth: 1,
          lineCap: .round,
          lineJoin: .round
        )
      )
    }
    .coordinateSpace(name: self.coordinateSpaceName)
    .gesture(
      DragGesture(coordinateSpace: .named(self.coordinateSpaceName))
        .onChanged { value in
          guard self.success == nil else { return }
          self.gestureLocation = value.location

          for (index, rect) in self.points.enumerated() {
            let bindArea: CGRect

            if self.selectedPoints.first == nil {
              bindArea = CGRect(
                x: rect.minX - 35,
                y: rect.minY - 35,
                width: rect.width + 70,
                height: rect.height + 70
              )

            } else {
              bindArea = CGRect(
                x: rect.minX - 22,
                y: rect.minY - 22,
                width: rect.width + 44,
                height: rect.height + 44
              )
            }

            if bindArea.contains(value.location) {
              if !selectedPoints.contains(index) {
                selectedPoints.append(index)

                withAnimation(.linear(duration: 0.2)) {
                  scalePoints.append(index)

                  DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    withAnimation(.linear(duration: 0.2)) {
                      scalePoints.removeAll { $0 == index }
                    }
                  }
                }
              }
            }
          }
        }
        .onEnded { _ in
          if selectedPoints.first != nil {
            if checkKey(selectedPoints.map { "\($0)" }.joined()) {
              success = true
              //Vibration.success.vibrate()
            } else {
              //Vibration.error.vibrate()
              withAnimation(.easeInOut(duration: 0.15)) {
                success = false
              }

              DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                withAnimation(.linear(duration: 0.15)) {
                  success = nil
                }
                selectedPoints = []
              }
            }
          }
          gestureLocation = nil
        }
    )
  }

  private let values = [[0, 1, 2], [3, 4, 5], [6, 7, 8]]
  private let coordinateSpaceName = "GraphicKeyView.coordinateSpaceName"

  @ViewBuilder
  private var circles: some View {
    GeometryReader { geometry in
      VStack(spacing: 0) {
        Color.clear
        VStack(spacing: 0) {
          ForEach(0..<3) { row in
            HStack(spacing: 0) {
              ForEach(0..<3) { column in
                Rectangle()
                  .fill(Color.clear)
//                  .overlay(
//                    GeometryReader { circleGeometry in
//                      Circle()
//                        .fill(color(for: values[row][column]))
//                        .onAppear {
//                          let frame = circleGeometry.frame(in: .named(coordinateSpaceName))
//                          points[values[row][column]] = frame
//                        }
//                        .onChange(of: circleGeometry.frame(in: .named(coordinateSpaceName))) { frame in
//                          points[values[row][column]] = frame
//                        }
//                    }
//                    .frame(width: 12, height: 12)
//                    .scaleEffect(scalePoints.contains(values[row][column]) ? 2 : 1)
//                  )
                  .frame(width: geometry.size.width / 3, height: geometry.size.width / 3)
              }
            }
          }
        }
      }
    }
  }

  public init(checkKey: @escaping (String) -> Bool) {
    self.checkKey = checkKey
  }
}

#if DEBUG

public struct GraphicPasswordPreviewView: View {

  public var body: some View {
    GraphicKeyView { _ in
      return true
    }
  }

  public init() {}
}

#endif
